...
const websiteItems = [
  ...
  { name: 'Home Page Banners', icon: Film, href: '/dashboard/banners' },
  { name: 'Push Alerts', icon: Bell, href: '/dashboard/notifications' },
  ...
  return (
    <>
      ...
      <aside className={`
        ...
        w-64 transition-all duration-300 transform
      ...
      `}>
        <div className="flex flex-col h-full p-5">
          ...
          {/* Navigation Links */}
          <nav className="flex-1 space-y-8 overflow-y-auto custom-scrollbar pr-2 pt-2">
                  ...
                  return (
                    <Link 
                      ...
                      className={`
                        ...
                        ${isActive 
                          ? 'bg-gradient-to-r from-orange-600/10 to-transparent border border-orange-500/20 text-orange-400' 
                          : 'text-gray-400 hover:text-white hover:bg-white/5 border border-transparent'}
                      `}
                    >
                      <div className="flex items-center gap-3">
                        <item.icon size={16} className={isActive ? 'text-orange-400' : 'text-gray-500 group-hover:text-white transition-colors'} />
                        <span className="text-xs font-semibold tracking-wide">{item.name}</span>
                      </div>
                      {isActive && <motion.div layoutId="activeDot" className="w-1 h-1 rounded-full bg-orange-500 shadow-[0_0_8px_rgba(249,115,22,0.5)]" />}
                    </Link>
                  );
                })}
              </div>
