...
async function migratePoojas() {
  ...
  if (allPoojas.length > 0) {
    batches.push([allPoojas[0]]);
    for (let i = 1; i < allPoojas.length; i += BATCH_SIZE) {
      batches.push(allPoojas.slice(i, i + BATCH_SIZE));
    }
  }
  ...
  for (let b = 0; b < batches.length; b++) {
    ...
    let sqlOutput = `-- Migration generated by script - Batch ${b + 1}\n\n`;

    for (const pooja of batch) {
    ...
    const outputPath = path.join(process.cwd(), 'docs', `poojas_migration_batch_${b + 1}.sql`);
    fs.writeFileSync(outputPath, sqlOutput);
    console.log(`Generated batch ${b + 1} with ${batch.length} entries at: ${outputPath}`);
  }
}
