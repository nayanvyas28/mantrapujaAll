...
const stateRegex = /"id":\s*"tn",\s*"path":\s*"([^"]+)"/;
const sMatch = stateRegex.exec(mapDataFile);
const poly = parseSVGPath(sMatch[1]);
console.log('Searching around 229, 657...');
for (let d = 1; d <= 20; d++) {
    for (let dx = -d; dx <= d; dx++) {
        for (let dy = -d; dy <= d; dy++) {
            if (isPointInPolygon([229 + dx, 657 + dy], poly)) {
                console.log(`Found valid point: x=${229 + dx}, y=${657 + dy} (dist=${d})`);
                process.exit(0);
            }
        }
    }
}
console.log('No valid point found in d=20');
