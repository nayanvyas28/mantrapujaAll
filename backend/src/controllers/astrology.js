...
const proxyAstroRequest = async (req, res) => {
    ...
    for (let i = 0; i < API_KEYS.length; i++) {
        ...
        try {
            ...
            const errorBody = await response.json().catch(() => ({ msg: "Internal Error" }));
            const status = response.status;
            const errorMsg = errorBody.msg || response.statusText;
            ...
            if (status === 429 || status === 401 || (status >= 500)) {
                ...
                lastError = error;
                console.error(`[AstroProxy] Key #${i+1} Exception:`, error.message || error);
                continue;
            }
        }
