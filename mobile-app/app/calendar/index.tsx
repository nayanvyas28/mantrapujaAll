...
import { ArrowLeft, Calendar as CalIcon, Settings2, Ticket, ChevronRight } from 'lucide-react-native';
...
export default function CalendarScreen() {
    ...
    const [viewMode, setViewMode] = useState<'festivals' | 'calendar'>('calendar');
        ...
        <View style={[styles.container, { backgroundColor: 'transparent' }]}>
            ...
            <ScrollView contentContainerStyle={styles.scrollContent}>
                        ...
                        <TouchableOpacity activeOpacity={0.8} onPress={() => router.push('/profile/bookings')}>
                            <Card variant="solid" style={styles.bookingCard}>
                                ...
                                <View style={styles.bookingInfo}>
                                    <Typography variant="body" style={{ fontWeight: 'bold' }} color={colors.foreground}>
                                        {latestBooking.pooja_name}
                                    </Typography>
                                    <Typography variant="bodySmall" color={colors.saffron} style={{ marginTop: 4 }}>
                                        {t('calendar.confirmed', 'Confirmed')} • {latestBooking.pooja_date}
                                    </Typography>
                                </View>
                ...
                <View style={[styles.toggleWrapper, { backgroundColor: colors.card, borderColor: colors.borderMuted }]}>
                    ...
                    >
                        <Typography variant="label" color={viewMode === 'festivals' ? colors.foreground : colors.muted}>
                            {t('calendar.recent_upcoming', 'Recent / Upcoming')}
                        </Typography>
                    </TouchableOpacity>
                    <TouchableOpacity
                    ...
                    >
                        <Typography variant="label" color={viewMode === 'calendar' ? colors.foreground : colors.muted}>
                            {t('calendar.calendar_view', 'Hindu Calendar View')}
                        </Typography>
                    </TouchableOpacity>
                </View>
                        ...
                        {loading && festivals.length === 0 ? (
                            ...
                            <>
                                {(() => {
                                    ...
                                    const displayList = showAllFestivals ? upcomingFestivals : upcomingFestivals.slice(0, 4);

                                    return displayList.map((fest) => (
                                ...
                                {!showAllFestivals && festivals.length > 4 && (
                                    <TouchableOpacity
                                        activeOpacity={0.8}
                                        style={[styles.viewAllButton, { borderColor: colors.border }]}
                                        onPress={() => setShowAllFestivals(true)}
                                    >
                                ...
                                {showAllFestivals && (
                                    <View style={{ marginTop: 24, padding: 16 }}>
                                        <Typography variant="body" color={colors.mutedForeground} style={{ textAlign: 'center' }}>
                                            {"Don't see your festival? Currently we're only highlighting major events. Let us know what you'd like to see!"}
                                        </Typography>
                                    </View>
                                )}
                            </>
                    ...
                    <View style={styles.calendarContainer}>
                        ...
                        {selectedFestival && (
                            <TouchableOpacity
                                ...
                                onPress={() => router.push(`/calendar/${selectedFestival.id}`)}
                            >
                                <Card variant="glass" style={[styles.previewCard, { borderColor: colors.saffron }]}>
                                    <View style={styles.previewHeader}>
                                        <View style={styles.previewInfo}>
                                            <Typography variant="label" color={colors.saffron}>{selectedFestival.date} • {t('calendar.upcoming_label', 'UPCOMING')}</Typography>
                                            <Typography variant="h3" style={{ marginTop: 4 }}>{selectedFestival.name}</Typography>
                                            <Typography variant="bodySmall" color={colors.mutedForeground} numberOfLines={2} style={{ marginTop: 4 }}>
...
const styles = StyleSheet.create({
    ...
    scrollContent: {
        padding: 24, // Matches 24 padding from Grid calculation
        paddingBottom: 40,
    },
